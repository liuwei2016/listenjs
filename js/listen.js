...
(function() {
  ...
  var previousUnderscore = root._allyes;
  ...
  _.noConflict = function() {
    root._allyes = previousUnderscore;
    return this;
  };
  ...
  _.addClass = function(node,className) {
    if (!_.hasClass(node,className)) node.className += " "+className;
    return node;
  };
...
_.off = function(node,evtype,fn){
  if (node.removeEventListener) {
    node.removeEventListener(evtype,fn,false);
    return node;
  }
  else if(node.length && node[0].removeEventListener){
     _.each(function(i){
       node.removeEventListener(evtype,fn,false);
     });
    return node;
...
_.cookie = function(key, value, options) {
  ...
  _.ajax=function(args) {
    ...
    if( !args.async || args.async ===true){
      xhr.open(args.method,args.url,true); //异步请求
    }else{
      xhr.open(args.method,args.url,false); //同步请求
    }
